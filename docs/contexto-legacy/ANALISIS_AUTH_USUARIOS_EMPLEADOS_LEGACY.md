...
## 0. LÓGICA DE NEGOCIO: QUIÉN ES QUIÉN Y “DAR DE ALTA”

### ¿Los “usuarios” son las empresas?

**No.** En Paco:

- **Empresa (Company)** = cliente que contrata Paco. Vive en la tabla `companies`. **No tiene cuenta de login**: la empresa no se “loguea”. Es el contexto (empresa_id) para empleados, productos, roles por empresa, etc.
- **Usuario (User)** = persona que **sí puede iniciar sesión** (en el panel web o en la app). Vive en la tabla `users`. Hay **tres tipos** según el campo `type`.

Por tanto: **las empresas no son usuarios**. Quien entra al dashboard o a la app es siempre un registro de la tabla `users`.

---

### Los tres tipos de usuario y qué acceso dan

| Tipo en `users.type` | Quién es en la práctica | ¿Acceso al **dashboard** (panel Paco)? | ¿Acceso a la **app móvil**? | Dónde se crea |
|----------------------|-------------------------|----------------------------------------|-----------------------------|----------------|
| **user** | Personal de Paco / superadmin (sin empresa fija) | ✅ Sí (login web) | ❌ No | Catálogo **Usuarios** (UsersController) |
| **high_user** | Personal de la **empresa** que usa el panel (RH, gerente, etc.) | ✅ Sí (login web, con empresa) | ❌ No* | Catálogo **Registro de usuarios** (HighUsersController) |
| **high_employee** | **Trabajador** de la empresa (el que usa la app) | ❌ No | ✅ Sí (login app) | **Signup en la app** (o integración externa), no desde el panel |

\*El login del panel web está pensado para `user` y `high_user`. El tipo `high_employee` se usa para la app.

Resumen:

- **Dashboard (panel Paco):** solo `user` y `high_user`.
- **App móvil:** solo `high_employee` (usuario vinculado a un empleado).

---

### ¿Qué pasa cuando “la empresa da de alta a un empleado”?

En Paco hay **dos cosas distintas** que a veces se llaman “dar de alta”:

1. **Dar de alta al empleado en el catálogo (ficha del trabajador)**  
   - Lo hace alguien con acceso al **panel** (un `user` o `high_user`).  
   - Se usa el catálogo **Altas de empleados** (HighEmployeesController).  
   - Se crea un registro en `high_employees`: nombre, email, móvil, empresa, departamento, puesto, etc.  
   - **En este paso no se crea ninguna cuenta de login.** El empleado todavía no puede entrar a la app ni al dashboard.

2. **Dar de alta la “cuenta” para que el empleado entre a la app**  
   - El empleado ya debe existir en `high_employees` (paso 1).  
   - En el legacy, la cuenta se crea cuando el **propio empleado** hace **signup en la app**: introduce su email o móvil (que ya está en su ficha), código de validación si la empresa lo exige, y contraseña.  
   - Eso crea un registro en `users` con `type = high_employee` y `high_employee_id = id del empleado`.  
   - A partir de ahí ese usuario puede **loguearse en la app** (no en el dashboard).

Por tanto:

- **“La empresa da de alta a un empleado”** = normalmente es el **paso 1** (crear la ficha en Altas de empleados).  
- Eso **no** da por sí solo acceso al dashboard ni a la app.  
- **Acceso al dashboard** lo tiene quien la empresa registra como **usuario de panel** en **Registro de usuarios** (se crea un `high_user`).  
- **Acceso a la app** lo tiene el empleado cuando existe su **usuario** `high_employee`, creado vía signup en la app (o por integración tipo EmployeeHistoryController).

---

### Ejemplo completo con un empleado

- **Empresa:** “Acme SA” (id 5 en `companies`). No hace login.
- **Persona que gestiona el panel (RH):** María. Tiene un **User** con `type = high_user`, `company_id = 5`, roles “Recursos Humanos”. Se creó en **Registro de usuarios**. María entra al **dashboard** con email + contraseña.
- **Trabajador nuevo:** Juan. La empresa quiere que aparezca en el sistema y que pueda usar la app.

**Paso 1 – Alta del empleado (solo datos):**  
María entra al panel → **Altas de empleados** → Crear. Completa nombre, email, móvil, empresa Acme, departamento, puesto, etc. Se guarda un **HighEmployee** (id 101). Juan **aún no puede entrar a ningún sitio**.

**Paso 2 – Acceso a la app (cuenta para Juan):**  
Juan abre la **app**, pulsa “Registrarse”, pone su email (el mismo que tiene en la ficha). La app comprueba que existe un empleado con ese email; si la empresa pide código, lo pide; Juan pone contraseña. Se crea un **User** con `type = high_employee`, `high_employee_id = 101`. A partir de ahí Juan puede **loguearse en la app** (no en el dashboard).

**Paso 3 – (Opcional) Acceso al dashboard para otra persona:**  
Si Acme quiere que otra persona (p. ej. Pedro, jefe de planta) entre al **panel**, María va a **Registro de usuarios** → Crear → nombre Pedro, email, empresa Acme, contraseña, roles. Se crea un **User** con `type = high_user`, `company_id = 5`. Pedro puede entrar al **dashboard**; no es un “empleado” del catálogo Altas de empleados ni usa la app como empleado.

---

### Resumen por parte del catálogo admin

| Parte del catálogo (panel) | Qué se gestiona | Qué se crea / qué acceso da |
|----------------------------|------------------|-----------------------------|
| **Usuarios** (UsersController) | Usuarios del panel **sin empresa** (superadmin, backoffice Paco) | Crea **User** `type = user`. Acceso solo al **dashboard**. |
| **Registro de usuarios** (HighUsersController) | Usuarios del panel **con empresa** (RH, gerentes, etc.) | Crea **User** `type = high_user` con `company_id`. Acceso solo al **dashboard**. |
| **Altas de empleados** (HighEmployeesController) | Fichas de los **trabajadores** de cada empresa | Crea **HighEmployee** (tabla `high_employees`). **No crea User.** No da login por sí solo. |
| **App – Signup** (AuthController::signup) | Que un **empleado** tenga cuenta para la app | Crea **User** `type = high_employee` con `high_employee_id`. Acceso solo a la **app**. |

La **empresa** (Company) no es un usuario: es el cliente/organización. Quienes hacen login son siempre registros de `users` (`user`, `high_user` o `high_employee`).
...
## 8. ESTADO EN TECBEN-CORE (qué tenemos / qué falta)

Resumen comparado con los tres catálogos del panel legacy.

### Lo que ya tenemos

| Equivalente legacy | En tecben-core | Comentario |
|--------------------|----------------|------------|
| **Usuarios** (User `type = user`) | ✅ **Catálogo Usuarios** | En `UsuarioResource` se puede crear usuario con **tipo "Usuario básico (panel)"** (`tipo = user`). Sin empresa fija, acceso solo al panel. |
| **Registro de usuarios** (User `type = high_user`) | ✅ **Catálogo Usuarios** | En el mismo `UsuarioResource`, tipo **"Administrador de empresa"** (`tipo = admin`) con `empresa_id`. Equivale al usuario de panel con empresa (RH, gerente). |
| **Cuenta para la app** (User `type = high_employee`) | ✅ **Catálogo Usuarios** | Tipo **"Empleado (app)"** (`tipo = employee`) con `empleado_id`. Se crea el **Usuario** vinculado a un empleado existente; ese usuario puede usar la app. |

Es decir: **los 3 tipos de usuario** (user, admin, employee) se crean desde **un solo catálogo**, **Usuarios**. No hay dos pantallas separadas como en legacy (Usuarios vs Registro de usuarios); está unificado en un solo recurso con el campo `tipo` y opcionalmente `empresa_id` / `empleado_id`. **Sí tenemos la creación de los 3 "roles"/tipos de usuario** (en el sentido de los tres tipos de cuenta que describe el análisis).

- **Relación usuario–empleado:** ya existe: `Usuario.empleado_id` → `Empleado`, 1:1 opcional (como en el análisis).
- **Modelo Empleado:** existe (`App\Models\Empleado`, tabla `empleados`) con empresa, número_empleado, nombre, apellidos, email, teléfono, celular, curp, rfc, fechas, etc.

### Lo que nos hace falta

| Equivalente legacy | En tecben-core | Comentario |
|--------------------|----------------|------------|
| **Altas de empleados** (HighEmployeesController) | ❌ **Falta** | En legacy es el catálogo donde se crea **solo la ficha del trabajador** (HighEmployee): nombre, email, móvil, empresa, departamento, puesto, etc. **No crea User.** En tecben-core **no existe un recurso Filament (CRUD) para dar de alta empleados**. El modelo `Empleado` existe, pero no hay pantalla en el panel para crear/editar/listar empleados. |

Consecuencia: para crear un **Usuario** tipo "Empleado (app)" hay que elegir un `empleado_id` en el formulario de Usuarios; si no hay forma de dar de alta empleados, no se puede completar ese flujo desde el panel. **La alta de empleado (catálogo "Altas de empleados") es lo que falta.**

### Resumen

- **Los 3 tipos de usuario** (panel sin empresa, panel con empresa, app/empleado): **ya se pueden crear** desde **Usuarios** (un solo catálogo con `tipo` + `empresa_id`/`empleado_id`).
- **El catálogo "Altas de empleados"** (solo ficha del trabajador, sin crear usuario): **no está implementado**; hace falta un recurso Filament para el modelo `Empleado` (alta, edición, listado) para poder dar de alta empleados y después asignarles cuenta de app desde Usuarios si se desea.
